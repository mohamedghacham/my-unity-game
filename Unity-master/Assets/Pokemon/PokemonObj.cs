...
public class PokemonObj : MonoBehaviour {
	...
	public bool UseMove(Vector3 direction, Move move){
		...
		case MoveNames.Growl:{
			...
			GetComponent<AudioSource>().PlayOneShot((AudioClip)Resources.Load("Audio/Growl"));
			move.cooldown = 0;
		...
		case MoveNames.Tackle:{
			...
			GetComponent<Rigidbody>().AddForce(direction*range*GetComponent<Rigidbody>().mass*500);
			move.cooldown = 0;
			pokemon.pp-=move.GetPPCost();
			return true;}
